...
test "Read depth1 GIF image" {
    const file = try helpers.testOpenFile(helpers.zigimg_test_allocator, "tests/fixtures/gif/depth1.gif");
    defer file.close();
    ...
    try gif_file.read(helpers.zigimg_test_allocator, stream_source.reader(), stream_source.seekableStream(), &pixels_opt);
